...
void Destino::setNombre_provincia(const char *p) {
  strcpy(nombre_provincia, p);
}
...
void Destino::cargar(int idd) {
  ...
  while (true) {
    setColor(CYAN);
    locate(40, 10);
    cout << "Ingrese nombre de provincia: ";
    setColor(WHITE);
    locate(40, 11);
    cargarCadena(nombre_provincia, 39);
    ...
    setColor(RED);
    locate(40, 13);
    cout << "Provincia invalida. Intente nuevamente.";
    setColor(WHITE);
    anykey();
    LimpiarLineas(10, 15, 40);
  }
  ...
  setColor(WHITE);
  locate(40, 11);
  cargarCadena(nombre_destino, 39);
  LimpiarLineas(10, 15, 40);

  setColor(CYAN);
  locate(40, 10);
  cout << "Ingrese hora de duracion de viaje: ";
  setColor(WHITE);
  locate(40, 11);
